//
// Modification of GGAC Code for rapid tape movement and /home/data/ directory
//June, 8  2017
...
//      fitVec[i]=peak->Fit(histVec[i]);                        //NEED TO CHANGE STORAGE OR VECTOR TYPE
	   ...
	   //Histogram Names
		...
		//Save Histogram
		c1.Update();
		TString path("/home/data/cnatzke/SimulationResults/Data/RadiusVariation/AngularHistograms/Fitted/");
		path+=h_name;
		path+=".png";
		cout << path.Data() << endl;
		c1.SaveAs(path);
		...
		histroot.WriteTObject(histVec[i]);
        }
        else{}
	}
	return counts;
